            ...
            <xsl:for-each-group select="*" group-starting-with="h2">
                <xsl:choose>
                    <xsl:when test="current-group()[self::h2]">
                        <xsl:comment> sec lvl 2 begin </xsl:comment><sec lvl="1">
                                <xsl:for-each-group select="current-group()" group-starting-with="h3">
                                    <xsl:choose>
                                        <xsl:when test="current-group()[self::h3]">
                                            <xsl:comment> sec lvl 3 begin </xsl:comment><sec lvl="2">
                                                    <xsl:for-each-group select="current-group()" group-starting-with="h4">
                                                        <xsl:choose>
                                                            <xsl:when test="current-group()[self::h4]">
                                                                <xsl:comment> sec lvl 4 begin </xsl:comment><sec lvl="3">
                                                                    <xsl:for-each-group select="current-group()" group-starting-with="h5">
                                                                        <xsl:choose>
                                                                            <xsl:when test="current-group()[self::h5]">
                                                                                <xsl:comment> sec lvl 5 begin </xsl:comment><sec lvl="4">
                                                                                    <xsl:for-each-group select="current-group()" group-starting-with="h6">
                                                                                        <xsl:choose>
                                                                                            <xsl:when test="current-group()[self::h6]">
                                                                                                <xsl:comment> sec lvl 6 begin </xsl:comment><sec lvl="5">
                                                                                                    <xsl:for-each-group select="current-group()" group-starting-with="h7">
                                                                                                        <xsl:choose>
                                                                                                            <xsl:when test="current-group()[self::h7]">
                                                                                                                <xsl:comment> sec lvl 7 begin </xsl:comment><sec lvl="6">
                                                                                                                    <xsl:for-each select="current-group()">
                                                                                                                        <xsl:copy-of select="."/>
                                                                                                                    </xsl:for-each>
                                                                                                                </sec><xsl:comment> sec lvl 7 end </xsl:comment>
                                                                                                            </xsl:when>
                                                                                                            <xsl:otherwise>
                                                                                                                <xsl:for-each select="current-group()">
                                                                                                                    <xsl:copy-of select="."/>
